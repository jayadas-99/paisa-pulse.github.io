...
import { auth, firebaseReady, googleProvider } from "../services/pulseFirebase";
import { getPulseProfile, savePulseProfile } from "../services/pulseDataService";

export const PulseAuthContext = createContext(null);
...
export function PulseAuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, (firebaseUser) => {
      ...
      withTimeout(getPulseProfile(firebaseUser.uid))
  ...
  async function signup(email, password, name, salaryDate, monthlyIncome) {
    ...
    await savePulseProfile(credential.user.uid, nextProfile);
  ...
  async function updateUserProfile(values) {
    ...
    await savePulseProfile(user.uid, nextProfile);
    setProfile(nextProfile);
  }
  ...
  return <PulseAuthContext.Provider value={value}>{children}</PulseAuthContext.Provider>;
}
